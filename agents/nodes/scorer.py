...
def extract_jd_sections(jd_text: str, max_chars: int = 1500) -> str:
    ...
    return extracted if len(extracted) > 500 else jd_text[:max_chars] #If extracted text is too-short we fall-back to the start of the JD
...
def passess_rule_filter(job: JobListing, profile: Dict[str, Any]) -> bool:
    """ 
        ...
        Checks for core-skills
    """
    
    #Get the skills from the profile
    core_skills = [s.lower() for s in profile.get("skills", {}).get("core", [])]
    if not core_skills:
        return True 
    
    jd_lower = job.jd_text.lower()
    
    #If none of the core-skills are in the JD, drop the Job
    return any(skill in jd_lower for skill in core_skills)
...
async def score_batch_with_llm(jobs: List[JobListing], profile: Dict[str, Any]) -> List[ScoredJob]:
    ...
    profile_summary = f""" 
        ...
        Core Skills: {', '.join(profile.get('skills', {}).get('core', []))}
        Secondary Skills: {', '.join(profile.get('skills', {}).get('secondary', []))}
        Other skills: {', '.join(profile.get('skills', {}).get('other', []))}
    """
...
async def score_listing(jobs: List[JobListing], profile: Dict[str, Any], user_id: int = None) -> List[ScoredJob]:
        ...
        try:
            for job in filtered_jobs:
                ...
                if db_job and db_job.score:
                    cached_count += 1   #Cache-Hit, job is present in the DB
                else:
    ...
    logger.info(f"Scorer: cache hit for {cached_count} jobs. {len(jobs_to_score)} need LLM Scoring")
